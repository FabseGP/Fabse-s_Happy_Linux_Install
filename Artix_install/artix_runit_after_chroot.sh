...
# Parameters
  ...
  DRIVE_LABEL="$(awk -F'"' '/^DRIVE_LABEL_after_chroot=/ {print $2}' artix_runit_pre_chroot.sh)"
  ENCRYPTION_choice="$(awk -F'"' '/^ENCRYPTION_after_chroot=/ {print $2}' artix_runit_pre_chroot.sh)"
  ...
  BOOTLOADER_label=""
  UUID_1=$(lsblk -no TYPE,UUID "$DRIVE_LABEL" | awk '$1=="part"{print $2}')
...
# Regenerating initramfs with encrypt-hook + keyfile
  ...
  if [ "$ENCRYPTION_choice" == "1" ]; then
    ...
    echo
    cryptsetup --batch-mode luksAddKey "$DRIVE_LABEL" /.secret/crypto_keyfile.bin
