...
# Setting up GRUB
  ...
  if [ "$ENCRYPTION_choice" == "1" ]; then
    ...
    sed -i 's/GRUB_CMDLINE_LINUX_DEFAULT="loglevel=3 quiet"/GRUB_CMDLINE_LINUX_DEFAULT="lsm=landlock,lockdown,yama,bpf\ loglevel=3\ quiet\ cryptdevice=UUID='$UUID_1':cryptroot:allow-discards\ root=UUID='$UUID_1'\ cryptkey=rootfs:\/.secret"/' hejsa.txt
